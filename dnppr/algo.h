...
//    cout<<rw_max_size<<endl;
    int chunk = (graph.n + thread_nums - 1) / thread_nums;
//    vector<double> thread_time(thread_nums);
    // rw_idx/size/offset for each thread
    vector<vector<int>> rw_idx_private(thread_nums);
    vector<vector<int>> rw_idx_size_private(thread_nums);
    vector<vector<int>> rw_idx_offset_private(thread_nums);
    for (int i = 0; i < thread_nums; ++i) {
        rw_idx_private[i].reserve((rw_max_size + thread_nums - 1) / thread_nums);
...
void init_container(){
    fwd_idx.resize(thread_nums);
    virtual_fwd_idx.resize(thread_nums);
    queues.resize(thread_nums);
    for (int i = 0; i < thread_nums; ++i) {
...
#pragma omp for schedule(static) nowait
    for (int i = 0; i < childrens.size(); ++i) {
        ...
        assert(omp_get_num_threads()==thread_nums);
        string &child = childrens[i];
...
void Smacof( const vector<double>& Weights, const vector<double>& Distances, const double& tol, const unsigned& maxloops, Coordinate2d& InitialZ ) {
    ...
    if(M>100){
        Eigen::initParallel();
        Eigen::setNbThreads(thread_nums);
    }
